...
void myFunc(const char *dir_path, int depth, long size, const char *pattern, int max_depth, int fileDetails, const char *commandForEachFile, const char *commandWithFileList, char fileTypeFilter) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (fileTypeFilter == 'f' && !fileCheckerP(full_path)) {
            ...
            continue;
        } else if (fileTypeFilter == 'd' && !S_ISDIR(entry->d_type)) {
        ...
        if (S_ISDIR(entry->d_type)) {
            if (max_depth > 0 && depth >= max_depth) {
                // Skip if the maximum depth is reached for directories
                continue;
            }
        }

        if (S_ISREG(entry->d_type) && size >= 0 && size < (long)entry->d_reclen) {
            // Skip regular files that do not meet the size criteria
            continue;
        }

        if (pattern != NULL && !strstr(entry->d_name, pattern)) {
