...
void myFunc(const char *dir_path, int depth, long size, const char *pattern, int max_depth, int fileDetails);
...
int main(int argc, char *argv[])
{
    ...
    myFunc(stDir, 0, size, patsr, max_depth, fileDetails);
    if (commandForEachFile)
    {
        executeCommandForEachFile(commandForEachFile, stDir, fileDetails, commandP);
    }
        ...
        if (fileCheckerP(full_path))
        {
            ...
            )
            {
                ...
                else
                {
                    ...
                    {
                        // Only print the filename if it's not a directory
        ...
        else
        {
            ...
            if (stat(full_path, &path_stat) == 0 && S_ISDIR(path_stat.st_mode))
            {
                if (depth < max_depth)
                {
                    ...
                    printf("%s\n", entry->d_name);
                    myFunc(full_path, depth + 1, size, pattern, max_depth, fileDetails);
                }
            }
        }
    }
        ...
        if (child_pid == 0) {
            ...
            if (WIFEXITED(status)) {
                ...
                if (exit_status == 0) {
                    ...
                    printf("Command execution failed (Exit status: %d).\n", exit_status);
                }
            }
        }
    }
...
void executeCommandForEachFile(const char *command, const char *path, int fileDetails, CommandFuncPointer commandP) {
    if (command) {
        pid_t child_pid = fork();
        if (child_pid == -1) {
            perror("fork");
